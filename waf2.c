...
static ngx_str_t ngx_http_get_x_forwarded_for_ip(ngx_http_request_t *r) {
    ngx_table_elt_t *x_forwarded_for;
    u_char *p, *end;

    // 查找 X-Forwarded-For 头
    x_forwarded_for = r->headers_in.x_forwarded_for;
    if (x_forwarded_for == NULL || x_forwarded_for->value.len == 0) {
        return r->connection->addr_text;
    }

    p = x_forwarded_for->value.data;
    end = p + x_forwarded_for->value.len;
    ...
    ngx_str_t ip;
    ip.data = x_forwarded_for->value.data;
    ip.len = p - x_forwarded_for->value.data;
...
static void ngx_http_log_reject(ngx_http_request_t *r, const char *reason, ngx_http_reject_rule_loc_conf_t *rlcf) {
    ngx_log_t *log;
    ngx_fd_t fd;
    ngx_str_t client_ip = ngx_http_get_x_forwarded_for_ip(r);
    ...
    ngx_log_error(NGX_LOG_ERR, log, 0, "Request rejected: %s, IP: %V, Port: %d, URI: %V, User-Agent: %V",
                  reason, &client_ip, ntohs(r->connection->sockaddr->sa_family == AF_INET ?
...
static ngx_int_t ngx_http_reject_rule_handler(ngx_http_request_t *r) {
    ...
    if (rlcf->ip_blacklist != NGX_CONF_UNSET_PTR) {
        ...
        for (i = 0; i < rlcf->ip_blacklist->nelts; i++) {
            if (ngx_strncmp(client_ip.data, ips[i].data, ips[i].len) == 0) {
